...
def to_list_rev (l : Turing.ListBlank Γ) : List Γ := by
  apply l.liftOn (fun l ↦ l.reverse.dropWhile (fun c ↦ c == Γ.two))
  rintro a _ ⟨i, rfl⟩
  cases a <;> simp
  . tauto
  . induction i
...
theorem to_list_rev_rfl (l : List Γ) : to_list_rev (Turing.ListBlank.mk l) = l.reverse.dropWhile (fun c ↦ c == Γ.two) := rfl
